...
export function IndexSectionHeader({ title, subtitle, className, as: Tag = 'h2' }: IndexSectionHeaderProps) {
    return (
        <div className={cn("relative flex items-center gap-3 sm:gap-6 mb-12 sm:mb-16 px-2 sm:px-4 group", className)}>
            {/* Coordinate Label */}
            <div className="hidden lg:block absolute -left-20 top-1/2 -translate-y-1/2 whitespace-nowrap">
                <span className="font-mono text-[9px] text-muted-foreground/30 rotate-180 [writing-mode:vertical-lr] tracking-[0.5em] uppercase">
            ...
            <div className="flex flex-col relative shrink-0">
                ...
                <Tag className="bg-foreground text-background px-5 py-2 sm:px-8 sm:py-3 font-mono text-[11px] sm:text-[14px] font-black uppercase tracking-widest index-cut-tr w-fit relative z-10">
                    {title}
                </Tag>
                {subtitle && (
                    <span className="font-mono text-[10px] sm:text-[13px] uppercase text-primary/60 mt-3 sm:mt-4 tracking-[0.2em] sm:tracking-[0.3em] font-medium animate-in fade-in slide-in-from-left-4 duration-500">
            ...
            <div className="hidden md:flex flex-col items-end font-mono text-[9px] text-muted-foreground/40 uppercase tracking-widest gap-1.5">
                <span>INDEX_PTR_REF: 0x{title.length}FF</span>
                <span className="text-primary/20 italic font-bold">ST_READY</span>
            </div>
        </div>
    )
}
