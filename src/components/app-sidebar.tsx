        ...
        <SidebarHeader className="py-6 bg-zinc-950 transition-all">
          <div className="flex items-center justify-between sm:justify-center gap-2 px-4 overflow-hidden">
            <div className="flex gap-2 items-center">
              ...
              <span className="text-xl font-bold truncate group-data-[collapsible=icon]:opacity-0 transition-opacity duration-200">
                    ...
                    <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild tooltip={item.title}>
                        <Link
                          prefetch
                          href={item.url}
                          className={`h-12 group flex items-center gap-3 group-data-[collapsible=icon]:ml-0 ml-3.5 px-3.5 py-2.5 hover:bg-primary/30 rounded-lg text-sm transition-all duration-300 ${
                            isActive
                              ? "bg-primary/20 text-white font-medium"
                              : "text-gray-400 hover:text-white "
                          }`}
                        >
                          <item.icon className="shrink-0 stroke-current" />
                          <span className="truncate">{item.title}</span>
                          {isActive && (
                            <div className="group-data-[collapsible=icon]:hidden absolute -left-1 top-0.5 w-1 h-10 bg-primary rounded-r-full"></div>
                          )}
                        </Link>
                      </SidebarMenuButton>
                    </SidebarMenuItem>
              ...
              <div className="px-7 py-2 flex flex-wrap gap-2">
                {[
                  ...
                  <Link
                    key={genre}
                    href={`/genres/${genre}`}
                    className="px-2.5 py-1 text-xs rounded-full hover:bg-primary hover:text-white bg-primary/20 text-primary transition-all"
                  >
                    {genre}
                  </Link>
                ))}
