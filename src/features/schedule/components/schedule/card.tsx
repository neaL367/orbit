...
/* -------------------------------------------------------------------------------------------------
   CoverImage — extracted + memoized so ScheduleCard doesn't re-render on image load
---------------------------------------------------------------------------------------------------*/
  ...
  return (
    <div className="relative w-full h-36 rounded-lg overflow-hidden ring-1 ring-zinc-800/50 group-hover:ring-zinc-700/50 transition-all">
      <div
        ...
        style={{ backgroundColor: coverColor, opacity: loaded ? 0 : 1 }}
      />
...
function ScheduleCardComponent({
  ...
  const mediaData = useMemo(() => {
    return {
      ...
      coverImageObj: media.coverImage,
      format: media.format,
  ...
  // Streaming links (these should ideally be stable)
  const streamingLinks = useMemo(
    () => getStreamingLinks(schedule),
    [schedule, getStreamingLinks]
  );

  // Responsive srcset
  const coverSrcSet = useMemo(() => {
    if (!coverImage) return undefined;
    const obj = mediaData.coverImageObj;
    const sizes = [];
    if (obj?.medium) sizes.push(`${obj.medium} 250w`);
    if (obj?.large) sizes.push(`${obj.large} 350w`);
    if (obj?.extraLarge) sizes.push(`${obj.extraLarge} 450w`);
  ...
  const status = useMemo(() => {
    ...
    const elapsed = now - airingAt;

    if (elapsed < 0) return "upcoming";

    const durationSeconds = (mediaData.duration || 30) * 60;
    const episodeEnd = airingAt + durationSeconds;
    const finishGrace = episodeEnd + 1800;

    if (now > finishGrace) return "finished";
    if (elapsed >= 0 && now < episodeEnd) return "airing";
    if (now >= episodeEnd && now <= finishGrace) return "finished";
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {status === "upcoming" &&
                ...
                timeUntilAiring > 0 && (
                  <Badge
                    variant="outline"
                    className="text-[10px] sm:text-xs px-2 py-1 border-blue-500/40 text-blue-400 bg-blue-500/10"
                  >
                    <Clock className="h-3 w-3 mr-1.5" />
                    {timeUntilAiringFormatted}
                  </Badge>
                  ...
                  {link.icon ? (
                    <div className="relative w-3.5 h-3.5 shrink-0">
                      ...
                      <img
                        ...
                        style={{
                          filter: `drop-shadow(0 0 2px ${color}90)`,
                        }}
                      />
                    </div>
                  ) : (
...
/* -------------------------------------------------------------------------------------------------
   Memoized wrapper — same logic you had
---------------------------------------------------------------------------------------------------*/
export const ScheduleCard = memo(ScheduleCardComponent, (prev, next) => {
  if (prev.schedule.id !== next.schedule.id) return false;
  if (prev.schedule.airingAt !== next.schedule.airingAt) return false;
  if (prev.schedule.episode !== next.schedule.episode) return false;
  ...
  const prevTitle =
    pm.title?.userPreferred || pm.title?.romaji || pm.title?.english;
  const nextTitle =
    nm.title?.userPreferred || nm.title?.romaji || nm.title?.english;

  if (prevTitle !== nextTitle) return false;

  const prevCover =
    pm.coverImage?.medium || pm.coverImage?.large || pm.coverImage?.extraLarge;
  const nextCover =
    nm.coverImage?.medium || nm.coverImage?.large || nm.coverImage?.extraLarge;

  if (prevCover !== nextCover) return false;

  if (pm.coverImage?.color !== nm.coverImage?.color) return false;
  if (pm.format !== nm.format) return false;
  if (pm.duration !== nm.duration) return false;

  if (prev.formatTime !== next.formatTime) return false;
  if (prev.getStreamingLinks !== next.getStreamingLinks) return false;
